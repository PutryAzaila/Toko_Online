...
//        VALUES ('".$_SESSION['id_pelanggan']."', '3', '".$tgl_transaksi."')");
//
//    if ($query_transaksi) {
...
session_start();
include "koneksi.php";
$cart = @$_SESSION['cart']; //cart adl var menyimpan data array
if (count($cart) > 0) {//jika jumlah cart lebih dari 0 maka akan menampilkan 
    $tgl_transaksi = date('Y-m-d');
    $lama_pengiriman = 2;
    ...
    if ($query_transaksi) {
        ...
        foreach ($cart as $key => $value) {
            ...
            $subtotal = $qty*$harga;
            mysqli_query($koneksi, "INSERT INTO detail_transaksi (id_transaksi, id_produk, qty, subtotal)
                VALUES ('".$id."', '".$value['id_produk']."', '".$qty."', '".$subtotal."')");//supaya bisa membeli produk lebih dari satu id
        }
        unset($_SESSION['cart']);
        echo "<script>alert('Anda Berhasil Membeli Produk');location.href='pembelian.php'</script>";
    }
